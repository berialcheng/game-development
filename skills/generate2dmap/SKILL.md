...
When the target project has `docs/ai_asset_register.md`, `assets/generated_placeholders`, or an asset lifecycle policy, treat `Expected Deliverables` paths as logical deliverables. Store generated candidates as a managed bundle first:

```text
assets/generated_placeholders/<map-or-stage-run-id>/
  source/       # raw generated images, prompt files, visible reference inputs
  processed/    # cleaned base layers, extracted props, normalized parallax layers
  preview/      # dressed-reference, stage-reference, layered-preview, QA composites
  asset-manifest.json
```

Create or update `asset-manifest.json` with `asset_id`, `source_skill`, lifecycle state, map mode, visual model, runtime object model, prompt files, source files, processed files, preview files, runtime candidates, collision/object metadata, and QC notes.

Only files listed with `state: accepted_for_runtime` may be copied into `assets/runtime_candidates/maps`, `assets/runtime_candidates/props`, `assets/runtime_candidates/tilesets`, or project-native runtime folders. `dressed-reference`, `stage-reference`, `layered-preview`, raw image outputs, prompt files, and rejected generations are planning/provenance artifacts, not runtime backgrounds or props.
...
- `asset-manifest.json` exists for managed generated bundles and marks runtime candidates explicitly
...
- Godot/runtime import candidates exclude raw outputs, reference mockups, QA previews, and files not marked `accepted_for_runtime`
