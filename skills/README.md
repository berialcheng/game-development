...
| `generate-game-audio` | `skills/generate-game-audio` | Generate and QC game music, ambience, stingers, UI sounds, and gameplay SFX candidates with a managed engine handoff. |
...
Prefer the shortest specialist path. Public external evidence goes to `game-reference-research`, synthetic/current-build evidence to `synthetic-gameplay-review`, user-supplied first-party or private human feedback to `synthesize-playtest-feedback`, and clear music/SFX asset production to `generate-game-audio`. Ask about uncertain source context only when it changes the route or claim. A review followed by a small scoped fix may continue directly to implementation; use `game-production-orchestrator` only when coordination materially helps.
