...
- Prompt provenance is mandatory for generated assets. Before calling `image_gen`, create the bundle `source/` folder and write the exact prompt you send to `image_gen` to `source/prompt-used.txt`. Do not rely on chat/session logs as the only prompt record.
- When postprocessing a generated image, always pass `--prompt-file <bundle>/source/prompt-used.txt` to `scripts/generate2dsprite.py process`. The processor treats missing prompt provenance as an error unless `--allow-missing-prompt` is explicitly used for legacy or non-generated assets.
...
When the target project has `docs/ai_asset_register.md`, `assets/generated_placeholders`, or an asset lifecycle policy, write generated output as a bundle before Godot import:

```text
assets/generated_placeholders/<asset_id-or-run_id>/
  source/       # raw-sheet.png, prompt-used.txt, layout guides, original image_gen output
  processed/    # sheet-transparent.png, extracted frames, cleaned prop PNGs
  preview/      # animation.gif and visual QC images
  asset-manifest.json
```

Create or update `asset-manifest.json` for every accepted run. Include `asset_id`, `source_skill`, `lifecycle_state`, `intended_use`, `art_style`, prompt/source/processed/preview files, runtime candidate entries, QC metadata, human review status, and rights notes.

Only files listed in the manifest with `state: accepted_for_runtime` may be copied into `assets/runtime_candidates/sprites`, `assets/runtime_candidates/props`, or `assets/runtime_candidates/fx` for Godot import. Keep raw images, prompts, layout guides, GIFs, rejected outputs, and `pipeline-meta.json` as provenance or QA evidence, not runtime art.

Update `docs/ai_asset_register.md` when present, linking the generated bundle and manifest. Never write generated output directly into `assets/final`.
## Workflow
...
Run `scripts/generate2dsprite.py process --prompt-file <bundle>/source/prompt-used.txt` on the raw image.
...
Treat missing `prompt-used.txt` as a failed bundle, not a minor documentation gap. Repair it before reporting the asset as accepted for runtime.
...
- `asset-manifest.json` with per-action runtime candidate entries
...
- `$game-production-orchestrator/references/asset-lifecycle.md`: cross-skill lifecycle and manifest contract when generating assets for a managed game project
